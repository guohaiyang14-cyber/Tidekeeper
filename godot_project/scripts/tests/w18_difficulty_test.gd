...
func _test_config_loaded() -> void:
	...
	_assert(cfg.get("default_tier", "") == "watcher", "默认档位=守夜人")
	_assert(cfg.has("teaching"), "含 教学宽容 配置")
	var teach: Dictionary = cfg.get("teaching", {})
	_assert(int(teach.get("nights", 0)) == 6, "教学夜窗口=6")
...
func _test_default_tier() -> void:
	DifficultySystem.reset_tier()
	_assert(DifficultySystem.get_tier() == "watcher", "当前档位=守夜人")
...
func _test_watcher_tier() -> void:
	...
	_assert(DifficultySystem.get_tier() == "lighthouse", "切换=灯塔")
...
func _test_teaching_night() -> void:
	_assert(DifficultySystem.is_teaching_night(6), "夜6=教学夜")
	_assert(not DifficultySystem.is_teaching_night(7), "夜7=非教学夜")
	# 灯塔 + 教学夜 2 → 0.6
	DifficultySystem.set_tier("lighthouse")
