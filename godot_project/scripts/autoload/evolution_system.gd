...
## 尝试发放进化道具；受软上限与「可进化项」约束。实际发放数量。
func grant_items(amount: int, respect_evolvable: bool = true) -> int:
	if amount <= 0:
		return 0
	var rules: Dictionary = ConfigLoader.get_evolution_rules()
...
## 精英击杀掉落（当夜精英池合计最多 1；第 5 夜命名精英必掉）
func try_elite_drop(night: int, is_named_elite: bool) -> int:
	...
	if not guaranteed and not has_evolvable_owned():
		return 0
	var n: int = grant_items(1, true)
	if n > 0:
		_elite_drops_this_night += n
	return n


## Boss 击杀掉落（不计入精英上限；第 15/20 夜额外数量见 config）
func try_boss_drop(night: int) -> int:
	...
	return grant_items(amount, true)
