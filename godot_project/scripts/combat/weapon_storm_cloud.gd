...
func fire(target: EnemyBase) -> void:
	...
	if ranked.is_empty():
		_strike_at(hash, mgr, target.global_position, radius)
		return
	for e in ranked:
		_strike_at(hash, mgr, e.global_position, radius)


## 在 at 位置半径 radius 内所有敌人各造成一次命中伤害（暴击独立掷骰）
func _strike_at(hash: SpatialHash, mgr: WeaponManager, at: Vector2, radius: float) -> void:
	...
	for p in pts:
		if p is EnemyBase:
			...
			if enemy.is_dead():
				continue
			enemy.take_damage(roll_hit_damage(), false, false, weapon_id)
	_spawn_fx(mgr, at, radius)


## 视觉反馈：在落雷点放一个青蓝打击圈，让玩家看到雷暴云在生效（无可见弹道）
func _spawn_fx(mgr: WeaponManager, at: Vector2, radius: float) -> void:
	mgr.spawn_area_effect(at, radius, get_behavior_color("effect_color", "5ec8ff"))
